...
function getContent () {
	...
	$templateContent = new nwTemplate(TEMPLATE_LOG_VIEWER);
	...
	$templateContent->assign('LOG_VIEWER_HEADING_SERVICE', LOG_VIEWER_HEADING_SERVICE);
        $templateContent->assign('LOG_VIEWER_HEADING_SERVICEGROUP', LOG_VIEWER_HEADING_SERVICEGROUP);
        $templateContent->assign('LOG_VIEWER_HEADING_HOSTGROUP', LOG_VIEWER_HEADING_HOSTGROUP);
        $templateContent->assign('LOG_VIEWER_HEADING_CUSTOMVARIABLE', LOG_VIEWER_HEADING_CUSTOMVARIABLE);
	...
	// set order by
	if (!isset($p['order_by'])) $p['order_by'] = 'timestamp';
	...
	// set base query
	$query = 'select %s from notification_logs ' . $filter . ' %s';


	// get complete count
	$dbResult = queryDB(sprintf($query, 'count(*) cnt', null));
	...
	foreach ($dbResult as $row) {
		...
		$templateSubContent->assign('NOTIFICATION_RULE', $row['notification_rule']);
		$templateSubContent->assign('TIMESTAMP', $row['timestamp']);
		...
		$templateSubContent->assign('STATUS', $row['check_result']);
		$templateSubContent->assign('METHOD', $row['method']);
		$templateSubContent->assign('USER', $row['user']);
...
function getPageNavi ($allCount, $resultCount, $resultsPerPage, $page = null, $find = null) {
	...
	if ($page > $logs['pages_per_line']) {
		...
		$href = 'index.php?action=logs&amp;numresults=' . $resultsPerPage . '&amp;p=1';
		...
		$href = 'index.php?action=logs&amp;numresults=' . $resultsPerPage . '&amp;p=' . ($start - 1);
		if (!empty($find)) $href .= '&amp;find_value=' . $find;
		$content .= htmlA($href, '&lt;', 'class="textLink"') . '&nbsp;';
	...
	if ($end < $numPages) {
		...
		$href = 'index.php?action=logs&amp;numresults=' . $resultsPerPage . '&amp;p=' . ($end + 1);
		if (!empty($find)) $href .= '&amp;find_value=' . $find;
		$content .= htmlA($href, '&gt;', 'class="textLink"') . '&nbsp;';
		
		// add link for last page
		$href = 'index.php?action=logs&amp;numresults=' . $resultsPerPage . '&amp;p=' . $numPages;
		if (!empty($find)) $href .= '&amp;find_value=' . $find;
		$content .= htmlA($href, '&gt;&gt;', 'class="textLink"') . '&nbsp;';
